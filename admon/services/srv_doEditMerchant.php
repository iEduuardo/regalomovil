...
?>
<?php 
	if($_GET['idm'] && $_GET['idm']!="" && $_GET['do']==1){
		$_str_intern_storenumber  = $_GET['identificador']; 
		$_name		  = $_GET['nombre'] ;
		$_idMerchant  = $_GET['idm'] ;
		//------------------------------------
		$_sql = "SELECT str_descripcion FROM "._PREPOS."ctl_merchants WHERE ( str_descripcion = '".$_name."' OR str_intern_storenumber = '".$_str_intern_storenumber."') AND (id_merchant NOT IN(".$_idMerchant."))";
		...
		if($_numRows >=1){
			...
			$_sql2   = "UPDATE "._PREPOS."ctl_merchants SET  str_intern_storenumber='".$_str_intern_storenumber."', str_descripcion ='".$_name."' WHERE id_merchant = ".$_idMerchant;
